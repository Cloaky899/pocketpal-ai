...
The legacy `MobiGPT API Chat` workflow was also hardened in a separate focused change. Push validation now includes `manim-agent`, while emulator smoke tests are manual-dispatch-only and explicitly provision the Android `emulator` and `platform-tools` packages. This prevents an emulator SDK provisioning problem from blocking ordinary JavaScript, APK, and AAB validation and makes ARM64/emulator diagnostics an intentional infrastructure test.
